...
(function () {
    ...
    function genTagsFromClassObj(classObj) {
        ...
        for (i = 0; i < classObj.methods.length; i++) {
            ...
            tags.push(method.name + '\t' + classObj.filePath + '\t/^' + method.line + '$/' + TERM + '\t' + 'f' + '\tclass:' + classObj.fullClassName);
        }
        //properties
        for (i = 0; i < classObj.props.length; i++) {
            prop = classObj.props[i];
            tags.push(prop.name + '\t' + classObj.filePath + '\t/^' + prop.line + '$/' + TERM + '\t' + 'p' + '\tclass:' + classObj.fullClassName);
        }
        //statics have 'file:' extended property
        if (classObj.statics) {
            for (i = 0; i < classObj.statics.props.length; i++) {
                prop = classObj.statics.props[i];
                tags.push(prop.name + '\t' + classObj.filePath + '\t/^' + prop.line + '$/' + TERM + '\t' + 'p' + '\t' + 'file:' + '\tclass:' + classObj.fullClassName);
            }
            for (i = 0; i < classObj.statics.methods.length; i++) {
                method = classObj.statics.methods[i];
                tags.push(method.name + '\t' + classObj.filePath + '\t/^' + method.line + '$/' + TERM + '\t' + 'p' + '\t' + 'file:' + '\tclass:' + classObj.fullClassName);
            }
        }
